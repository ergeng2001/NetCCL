...
public:
    ...
    void flush_once() {// NOTE: it may flush nothing
        // flush();
        uint16_t complete_nb = rte_eth_tx_burst(portid, queueid, tx_bufs, tx_nb);
        tx_nb -= complete_nb;
        if(tx_nb && complete_nb) {
            for(uint16_t i = 0; i < tx_nb; i++) tx_bufs[i] = tx_bufs[i + complete_nb];
        } 
        flush_timer = 0;
    }

    size_t rx_sw_buffered_pkt_cnt() {
        return rx_nb - rx_iter;
    }

    size_t rx_hw_buffered_pkt_cnt() {
        return rte_eth_rx_queue_count(portid, queueid);
    }

    size_t rx_buffered_pkt_cnt() {
        return rx_sw_buffered_pkt_cnt() + rx_hw_buffered_pkt_cnt();
    }
    ...
    size_t tx_sw_buffered_pkt_cnt() {
        return tx_nb;
    }

#ifdef rte_eth_tx_queue_count
    size_t tx_hw_buffered_pkt_cnt() {
        return rte_eth_tx_queue_count(portid, queueid);
    }

    size_t tx_buffered_pkt_cnt() {
        return tx_sw_buffered_pkt_cnt() + tx_hw_buffered_pkt_cnt();
    }
#endif
